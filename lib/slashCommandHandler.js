...
class CommandHandler {
  ...
  async handle(message) {
    ...
    try {
      ...
      // ❌ 이펙트 호출 체크 제거 (이제 !@로만 호출)
      // 한글 명령어는 모두 일반 명령어로 처리
  ...
  async handleComboCall(message) {
    ...
    // 1️⃣ 먼저 콤보 확인 (activeChar.data.combos에서)
  ...
  async handleHelp(message) {
    const embed1 = new EmbedBuilder()
      ...
      .setDescription('시트 연동 기반 DX3 TRPG 봇입니다.')
      .addFields(
        {
          name: '📋 **포럼 설정**',
      ...
      .addFields(
        ...
        },
        {
          ...
          value: '> `!등침` / `!등장침식` - 등장 시 1d10 침식률 추가\n' +
                 ...
                 '> `!@"이펙트명"` - 이펙트 상세 보기\n' +
                 '> `!콤보확인` - 내 콤보 목록\n' +
                 '> 💡 **콤보와 이펙트는 시트에서 자동으로 읽어옵니다!**'
        },
        {
          name: '🔹 **로이스 & 타이터스**',
          value: '> `!타이터스 "이름"` - 로이스 → 타이터스 변환\n' +
                 '> 💡 **로이스는 시트에서 자동으로 읽어옵니다!**\n' +
                 '> 💡 타이터스 변환 시 시트에 자동으로 반영됩니다!'
        }
      );
    ...
    const embed3 = new EmbedBuilder()
      ...
      .addFields(
        ...
        {
          name: '💡 **시트 연동 가이드**',
          value: '> 1️⃣ 포럼 채널 만들기 (`!포럼설정`)\n' +
                 '> 2️⃣ Google Sheets 템플릿 복사\n' +
                 '> 3️⃣ 시트 등록 (`!시트등록 [URL]`)\n' +
                 '> 4️⃣ 캐릭터 지정 (`!지정 "이름"`)\n' +
                 '> 5️⃣ 게임 시작! 🎮\n\n' +
                 '> 📋 자세한 설정 방법: `GOOGLE_SHEETS_SETUP.md`\n' +
                 '> 🎯 PbP(Play by Post) 플레이에 최적화!'
        }
      )
